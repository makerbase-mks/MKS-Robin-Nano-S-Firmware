...
extern volatile uint8_t temper_error_type;

Temperature thermalManager;
extern volatile uint8_t loop_start;
// public:
...
uint8_t Temperature::soft_pwm_amount[HOTENDS],
  ...
  void Temperature::PID_autotune(const float temp, const int8_t hotend, const int8_t ncycles, const bool set_result/*=false*/) {
    ...
    float Ku, Tu,
      ...
      float watch_temp_target ;//= temp -
        #if ENABLED(THERMAL_PROTECTION_BED) /*&& ENABLED(PIDTEMPBED) */&& ENABLED(THERMAL_PROTECTION_HOTENDS) /*&& ENABLED(PIDTEMP)*/
            if((PIDTEMPBED)&&(PIDTEMP))
                watch_temp_target = temp - (hotend < 0 ? (WATCH_BED_TEMP_INCREASE + TEMP_BED_HYSTERESIS + 1) : (WATCH_TEMP_INCREASE + TEMP_HYSTERESIS + 1));
            else
            {
             #if ENABLED(THERMAL_PROTECTION_BED) /*&& ENABLED(PIDTEMPBED)*/
                if(PIDTEMPBED)
                    watch_temp_target = temp - (WATCH_BED_TEMP_INCREASE + TEMP_BED_HYSTERESIS + 1);
                else
                    watch_temp_target = temp - (WATCH_TEMP_INCREASE + TEMP_HYSTERESIS + 1);
             #else
                watch_temp_target = temp - (WATCH_TEMP_INCREASE + TEMP_HYSTERESIS + 1);
             #endif
            }
      ...
      //int8_t watch_temp_period;// =
      uint32_t watch_temp_period;
        #if ENABLED(THERMAL_PROTECTION_BED)/* && ENABLED(PIDTEMPBED)*/ && ENABLED(THERMAL_PROTECTION_HOTENDS) /*&& ENABLED(PIDTEMP)*/
            if((PIDTEMPBED)&&(PIDTEMP))
            {
                watch_temp_period = hotend < 0 ? WATCH_BED_TEMP_PERIOD : WATCH_TEMP_PERIOD;
            }
            else
            {
                #if ENABLED(THERMAL_PROTECTION_BED)/* && ENABLED(PIDTEMPBED)*/
                    if(PIDTEMPBED)
                      watch_temp_period = WATCH_BED_TEMP_PERIOD;
                    else
                      watch_temp_period = WATCH_TEMP_PERIOD;  
                #else
                    watch_temp_period = WATCH_TEMP_PERIOD;
                #endif
            }
        #endif
      int8_t watch_temp_increase; //=
        #if ENABLED(THERMAL_PROTECTION_BED) /*&& ENABLED(PIDTEMPBED) */&& ENABLED(THERMAL_PROTECTION_HOTENDS) /*&& ENABLED(PIDTEMP)*/
            if((PIDTEMPBED)&&(PIDTEMP))
            {
                watch_temp_increase = hotend < 0 ? WATCH_BED_TEMP_INCREASE : WATCH_TEMP_INCREASE;
            }
            else
            {
                #if ENABLED(THERMAL_PROTECTION_BED)
                    if(PIDTEMPBED)
                        watch_temp_increase = WATCH_BED_TEMP_INCREASE;
                     else
                        watch_temp_increase = WATCH_TEMP_INCREASE;
                #else
                    watch_temp_increase = WATCH_TEMP_INCREASE;
                #endif
            }
	...
	if (HAS_PID_FOR_BOTH) {
      if (hotend < 0)
        ...
        soft_pwm_amount[hotend] = bias = d = (PID_MAX) >> 1;}
	else if(PIDTEMP)	soft_pwm_amount[hotend] = bias = d = (PID_MAX) >> 1;
	else soft_pwm_amount_bed = bias = d = (MAX_BED_POWER) >> 1;
      ...
      if (temp_meas_ready) { // temp sample ready
        ...
        if (heating && input > temp) {
          if (ELAPSED(ms, t2 + 5000UL)) {
            ...
            #endif
			#endif
			if(HAS_PID_FOR_BOTH)	{if (hotend < 0) soft_pwm_amount_bed = (bias - d) >> 1; else soft_pwm_amount[hotend] = (bias - d) >> 1;}
			else if(PIDTEMP)		soft_pwm_amount[hotend] = (bias - d) >> 1;
			else					soft_pwm_amount_bed = (bias - d) >> 1;
...
void Temperature::manage_heater() {
  ...
  //#if ENABLED(HEATER_0_USES_MAX6675)
   if(TEMP_SENSOR_0 == sensors_MAX31855)
   {
    if (current_temperature[0] > min(HEATER_0_MAXTEMP, MAX6675_TMAX - 1.0)) 
    {
        if(current_temperature[0] == 1800)
        {
           current_temperature[0] = current_temper_bak;
        }
        else
        {
            max_temp_error(0);
   ...
   current_temper_bak = current_temperature[0];
  //#endif
...
#if 0//ENABLED(HEATER_0_USES_MAX6675)
...
void Temperature::init() {
  ...
  #if 0//MB(RUMBA) && (TEMP_SENSOR_0 == -1 || TEMP_SENSOR_1 == -1 || TEMP_SENSOR_2 == -1 || TEMP_SENSOR_BED == -1)
  ...
  #if 0//ENABLED(PIDTEMP) && ENABLED(PID_EXTRUSION_SCALING)
    last_e_position = 0;
  #endif
...
void Temperature::disable_all_heaters() {
...
#if 1//ENABLED(HEATER_0_USES_MAX6675)
  ...
  int Temperature::read_max6675() {
    ...
    for (uint8_t i = sizeof(max6675_temp); i--;) {
      ...
      max6675_temp |= SPI2_ReadWriteByte(0x00);
...
//void Temperature::isr() {
    ...
    case SensorsReady: {
      ...
      case MeasureTemp_1:
		    ...
		    if(gCfgItems.singleNozzle == 1)
				raw_temp_value[1] += uhADCxConvertedValue[1];	//TH2
			else
  ...
  if (!adc_sensor_state && ++temp_count >= OVERSAMPLENR) { // 10 * 16 * 1/(16000000/64/256)  = 164ms.
	...
	if(TEMP_SENSOR_0 == sensors_MAX31855)
		{
		temp_dir[0]=0;
		temp_dir[1]=TEMPDIR(1);       //mks_TC
		}
	else
		{
		temp_dir[0]=TEMPDIR(0);
		temp_dir[1]=TEMPDIR(1);       //mks_TC
		}
    ...
    for (uint8_t e = 0; e < COUNT(temp_dir); e++) {
      ...
      */
      if(PIDTEMP)
        heater_on = 0 < soft_pwm_amount[e];
      else
        heater_on = 0 < target_temperature[e];
      
      //if (rawtemp > maxttemp_raw[e] * tdir && heater_on) max_temp_error(e);
      if (rawtemp > maxttemp_raw[e] * tdir && loop_start) max_temp_error(e);
      ...
      */
      if(PIDTEMPBED)
        bed_on = 0 < soft_pwm_amount_bed;
      else
        bed_on = 0 < target_temperature_bed;
      
      //if (current_temperature_bed_raw GEBED bed_maxttemp_raw && bed_on) max_temp_error(-1);
      if (current_temperature_bed_raw GEBED bed_maxttemp_raw && loop_start) max_temp_error(-1);
...
#if 1//HAS_TEMP_HOTEND || HAS_TEMP_BED
