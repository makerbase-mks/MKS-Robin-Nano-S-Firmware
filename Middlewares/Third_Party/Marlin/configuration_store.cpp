...
void MarlinSettings::postprocess() {
  ...
  bool MarlinSettings::save() {
    ...
    EEPROM_WRITE(ver);     // invalidate data first//60633
    EEPROM_SKIP(working_crc); // Skip the checksum slot//

    working_crc = 0; // clear before first "real data"

    const uint8_t esteppers = COUNT(planner.axis_steps_per_mm) - XYZ;
    EEPROM_WRITE(esteppers);//8258

    EEPROM_WRITE(planner.axis_steps_per_mm);//7980
    EEPROM_WRITE(planner.max_feedrate_mm_s);//1676
    EEPROM_WRITE(planner.max_acceleration_mm_per_s2);//7124

    EEPROM_WRITE(planner.acceleration);//18104
    EEPROM_WRITE(planner.retract_acceleration);//58383
    EEPROM_WRITE(planner.travel_acceleration);//39038
    EEPROM_WRITE(planner.min_feedrate_mm_s);//32853
    EEPROM_WRITE(planner.min_travel_feedrate_mm_s);//26630
    EEPROM_WRITE(planner.min_segment_time_us);//24403
    EEPROM_WRITE(planner.max_jerk);//58100
    #if !HAS_HOME_OFFSET
      const float home_offset[XYZ] = { 0 };
    #endif
    EEPROM_WRITE(home_offset);//31956

    #if HOTENDS > 1
      // Skip hotend 0 which must be 0
      hotend_offset[X_AXIS][1]=mksCfg.hotnd_offset_x;
      hotend_offset[Y_AXIS][1]=mksCfg.hotnd_offset_y;
      for (uint8_t e = 1; e < HOTENDS; e++)
        LOOP_XYZ(i) EEPROM_WRITE(hotend_offset[i][e]);//23497
    #endif
    ...
    else
    {
      ...
      EEPROM_WRITE(leveling_is_on);//293
      EEPROM_WRITE(dummy); // z_offset7042
      EEPROM_WRITE(mesh_num_x);//4409
      EEPROM_WRITE(mesh_num_y);//2931
      for (uint8_t q = mesh_num_x * mesh_num_y; q--;) EEPROM_WRITE(dummy);//62039
    }
    ...
    #endif
    EEPROM_WRITE(zprobe_zoffset);//65490
    ...
    //#else
      dummy = 0.0;
      for (uint8_t q = 9; q--;) EEPROM_WRITE(dummy);//17426
    }
    //#endif
    ...
    if(BED_LEVELING_METHOD & AUTO_BED_LEVELING_BILINEAR)
    {
     ...
     // );
      ...
      EEPROM_WRITE(grid_max_x);            // 1 byte 19173
      EEPROM_WRITE(grid_max_y);            // 1 byte 23595
      EEPROM_WRITE(bilinear_grid_spacing); // 2 ints 65290
      EEPROM_WRITE(bilinear_start);        // 2 ints 26369
      EEPROM_WRITE(z_values);              // 9-256 floats 13966
    }
    //#else
    else
    {
    ...
    else
    {
      ...
      EEPROM_WRITE(ubl_active);//55445
      EEPROM_WRITE(storage_slot);//49541
    }
    ...
    if(MACHINETPYE & DELTA)
    {
      EEPROM_WRITE(delta_height);              // 1 float 57168
      EEPROM_WRITE(delta_endstop_adj);         // 3 floats 17082
      EEPROM_WRITE(delta_radius);              // 1 float 32275
      EEPROM_WRITE(delta_diagonal_rod);        // 1 float 29426
      EEPROM_WRITE(delta_segments_per_second); // 1 float 19535
      EEPROM_WRITE(delta_calibration_radius);  // 1 float 59209
      EEPROM_WRITE(delta_tower_angle_trim);    // 3 floats 12056
    }
    else
    {
    ...
    EEPROM_WRITE(lcd_preheat_hotend_temp);//11521
    EEPROM_WRITE(lcd_preheat_bed_temp);//6398
    EEPROM_WRITE(lcd_preheat_fan_speed);//15998
    ...
    for (uint8_t e = 0; e < MAX_EXTRUDERS; e++) {
      ...
      {
        if (e < HOTENDS) {
          EEPROM_WRITE(PID_PARAM(Kp, e));//28927
          EEPROM_WRITE(PID_PARAM(Ki, e));//40849
          EEPROM_WRITE(PID_PARAM(Kd, e));//61724
    ...
    {
      EEPROM_WRITE(thermalManager.bedKp);//31389
      EEPROM_WRITE(thermalManager.bedKi);//42790
      EEPROM_WRITE(thermalManager.bedKd);//28450
    }
    //#endif
    ...
    #endif
    EEPROM_WRITE(lcd_contrast);//6610
    ...
    #endif
    EEPROM_WRITE(autoretract_enabled);//20760
    EEPROM_WRITE(retract_length);//44101
    EEPROM_WRITE(retract_feedrate_mm_s);//62158
    EEPROM_WRITE(retract_zlift);//20435
    EEPROM_WRITE(retract_recover_length);//34486
    EEPROM_WRITE(retract_recover_feedrate_mm_s);//46846
    EEPROM_WRITE(swap_retract_length);//30785
    EEPROM_WRITE(swap_retract_recover_length);//60715
    EEPROM_WRITE(swap_retract_recover_feedrate_mm_s);//37822
    ...
    //
      ...
      EEPROM_WRITE(parser.volumetric_enabled);//3546
    ...
    #else
      val = 0;
      for (uint8_t q = 11; q--;) EEPROM_WRITE(val);//45911
    #endif
    ...
    #else
      thrs = 0;
      for (uint8_t q = 2; q--;) EEPROM_WRITE(thrs);//45395
    #endif
    ...
    //
    ...
    #else
      dummy = 0.0f;
      EEPROM_WRITE(dummy);//33019
      EEPROM_WRITE(dummy);//52891
    #endif
      ...
      for (uint8_t q = 3; q--;) EEPROM_WRITE(dummyui32);//24980
    #endif
    ...
    //
      ...
      for (uint8_t q = 27; q--;) 
      {
        EEPROM_WRITE(dummy);//48784//11186
        //SERIAL_ECHOPAIR("q:",q);
        //SERIAL_ECHOPAIR(" offset:",eeprom_index);
        //SERIAL_ECHOPAIR(" dummy:",dummy);
        //SERIAL_ECHOPAIR(" CRC:",working_crc);
        //SERIAL_EOL();
      }
    #endif
    ...
    #else
      dummy = 0.0f;
      for (uint8_t q = 3; q--;) EEPROM_WRITE(dummy);//16208
    #endif
    ...
    if (!eeprom_error) {
      ...
      EEPROM_WRITE(version);//28940
      EEPROM_WRITE(final_crc);//13287
  ...
  bool MarlinSettings::load() {
    ...
    if (strncmp(version, stored_ver, 3) != 0) {
      ...
      */
      ...
      read_data(eeprom_index, (uint8_t *)tmp1, (XYZ + esteppers) * sizeof(float),&working_crc);
      read_data(eeprom_index, (uint8_t *)tmp2, (XYZ + esteppers) * sizeof(float),&working_crc);
      read_data(eeprom_index, (uint8_t *)tmp3, (XYZ + esteppers) * sizeof(uint32_t),&working_crc);
...
void MarlinSettings::reset() {
  ...
  LOOP_XYZE_N(i) {
    planner.axis_steps_per_mm[i]          = tmp1[i < COUNT(tmp1) ? i : COUNT(tmp1) - 1];//pgm_read_float(&tmp1[i < COUNT(tmp1) ? i : COUNT(tmp1) - 1]);
    planner.max_feedrate_mm_s[i]          = tmp2[i < COUNT(tmp2) ? i : COUNT(tmp2) - 1];//pgm_read_float(&tmp2[i < COUNT(tmp2) ? i : COUNT(tmp2) - 1]);
    planner.max_acceleration_mm_per_s2[i] = tmp3[i < COUNT(tmp3) ? i : COUNT(tmp3) - 1];//pgm_read_dword_near(&tmp3[i < COUNT(tmp3) ? i : COUNT(tmp3) - 1]);
  }
 ...
 // #if ENABLED(X_DUAL_ENDSTOPS) || ENABLED(Y_DUAL_ENDSTOPS) || ENABLED(Z_DUAL_ENDSTOPS)
    ...
    if(Z_DUAL_ENDSTOPS==1)
    {
       if(Z_DUAL_ENDSTOPS_ADJUSTMENT!=0)
        z_endstop_adj =  Z_DUAL_ENDSTOPS_ADJUSTMENT;
       else
        z_endstop_adj =  0;
    }
  //#endif
