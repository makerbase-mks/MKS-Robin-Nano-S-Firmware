...
class Planner {
  ...
  public:
     ...
     *  extruder     - target extruder
     */
    FORCE_INLINE static void buffer_line(float rx, float ry, float rz, const float &e, const float &fr_mm_s, const uint8_t extruder) {
/*
			...
			if (gCfgItems.breakpoint_reprint_flg == 1) {
				if (rz < gCfgItems.breakpoint_z_pos) 
						{
								return;
						}
			}
...
*/
     ...
     // #endif
      //#if IS_KINEMATIC
        	...
        	inverse_kinematics(raw);
				if (gCfgItems.breakpoint_reprint_flg == 1) 
				{
					if (cart[Z_AXIS] < gCfgItems.breakpoint_z_pos) 
					{
						//current_position[E_AXIS]=cart[E_AXIS];
						//current_position[Z_AXIS]=cart[Z_AXIS];
						position[E_AXIS]=LROUND(cart[E_AXIS] * axis_steps_per_mm[E_AXIS_N]);
						return;
					}
				}
				//gCfgItems.breakpoint_reprint_flg = 0;
     ...
     */
    FORCE_INLINE static void set_position_mm(float rx, float ry, float rz, const float &e) {
      //#if PLANNER_LEVELING && IS_CARTESIAN
