...
void Planner::check_axes_activity() {
  ...
  void Planner::apply_leveling(float &rx, float &ry, float &rz) {
    ...
    {
      #if 0//ENABLED(ENABLE_LEVELING_FADE_HEIGHT)
        const float fade_scaling_factor = fade_scaling_factor_for_z(rz);
        if (!fade_scaling_factor) return;
      //#elif HAS_MESH
      #else
        float fade_scaling_factor;
        if(BED_LEVELING_METHOD & HAS_MESH)
            fade_scaling_factor = 1.0;
      #endif
...
*/        
        if(BED_LEVELING_METHOD & AUTO_BED_LEVELING_UBL)
            ...
            #if ENABLED(ENABLE_LEVELING_FADE_HEIGHT)
            rz += ( mbl.get_z(rx, ry,fade_scaling_factor));
            #else
            rz += ( mbl.get_z(rx, ry));
            #endif
        else if(BED_LEVELING_METHOD & AUTO_BED_LEVELING_BILINEAR)
            rz += bilinear_z_offset(raw) * fade_scaling_factor;
        else
            rz += 0;
        //#endif
    }
  ...
  void Planner::unapply_leveling(float raw[XYZ]) {
    ...
    if (leveling_active && fade_scaling_factor) {
      ...
      else
      {
        if((BED_LEVELING_METHOD == AUTO_BED_LEVELING_UBL))
        {
          raw[Z_AXIS] -= (ubl.get_z_correction(raw[X_AXIS], raw[Y_AXIS]) * fade_scaling_factor);
        }
        else if((BED_LEVELING_METHOD == MESH_BED_LEVELING))
        {
        ...
        {
            raw[Z_AXIS] -= (bilinear_z_offset(raw) * fade_scaling_factor);
        }
        else
        {
            raw[Z_AXIS] -= 0;
  ...
  { 
    if (WITHIN(moves_queued, 2, (BLOCK_BUFFER_SIZE) / 2 - 1)) {
      if (segment_time_us < min_segment_time_us) {
        ...
        const uint32_t nst = segment_time_us + LROUND((double)(2 * (min_segment_time_us - segment_time_us) / moves_queued));
...
void Planner::buffer_segment(const float &a, const float &b, const float &c, const float &e, const float &fr_mm_s, const uint8_t extruder) {
  ...
  if(MACHINETPYE & IS_KINEMATIC)
  {
    ...
    {
        if(gCfgItems.had_breakpoint)
        {
            gCfgItems.had_breakpoint=0;
            position[E_AXIS] = gCfgItems.breakpoint_e_position;
        }
    }
  }
  else
  {
	if (gCfgItems.breakpoint_reprint_flg == 1) {
		if (c < gCfgItems.breakpoint_z_pos) 
        {
            position[E_AXIS] = target[E_AXIS_N];
...
*/
  ...
  if(BED_LEVELING_METHOD&PLANNER_LEVELING)	{
  	...
  	raw[1] = cart[Y_AXIS];
  	raw[2] = cart[Z_AXIS];
	apply_leveling(raw[0],raw[1],raw[2]);
  	}
  else
  	{
	  raw[0] = cart[X_AXIS];
  ...
  */
  if(MACHINETPYE & IS_KINEMATIC)
	...
	if(MACHINETPYE == MORGAN_SCARA)
	  inverse_kinematics_MORGAN_SCARA(raw);
	else
	  inverse_kinematics(raw);
