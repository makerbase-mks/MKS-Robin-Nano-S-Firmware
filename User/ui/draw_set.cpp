...
//	int imgHeight = LCD_HEIGHT - titleHeight;	
	...
	if(gCfgItems.wifi_btn_state == 0)
	{
		...
		else
		{
			...
			if(mksCfg.machine_parameter == 1)buttonMachinePara.btnHandle  =  BUTTON_CreateEx(BTN_X_PIXEL+INTERVAL_V*2, BTN_Y_PIXEL+INTERVAL_H,BTN_X_PIXEL, BTN_Y_PIXEL,hSetWnd, BUTTON_CF_SHOW, 0, alloc_win_id());
			if(gCfgItems.func_btn1_display_flag != 0)
	...
	//if(gCfgItems.display_style != 0){
		if(mksCfg.machine_parameter == 1)BUTTON_SetBmpFileName(buttonMachinePara.btnHandle, "bmp_machine_para.bin",1);
		if(mksCfg.machine_parameter == 1)BUTTON_SetBitmapEx(buttonMachinePara.btnHandle,0,&bmp_struct,BMP_PIC_X,BMP_PIC_Y);
		if(gCfgItems.multiple_language != 0)
		{
	        	if(mksCfg.machine_parameter == 1)BUTTON_SetText(buttonMachinePara.btnHandle, set_menu.machine_para);
		}
	//}
