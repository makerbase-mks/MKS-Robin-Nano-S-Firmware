...
#define Z_APPLY_DIR(v,Q) do{ Z_DIR_WRITE(v); if(Z_DUAL_STEPPER_DRIVERS==1)Z2_DIR_WRITE(v); }while(0)
#if 0//ENABLED(Z_DUAL_ENDSTOPS)
  #define Z_APPLY_STEP(v,Q) DUAL_ENDSTOP_APPLY_STEP(Z,v)
#else
  #define Z_APPLY_STEP(v,Q) if(Z_DUAL_ENDSTOPS==1){DUAL_ENDSTOP_APPLY_STEP(Z,v)}else{do{ Z_STEP_WRITE(v); if(Z_DUAL_STEPPER_DRIVERS==1)Z2_STEP_WRITE(v); }while(0);}
#endif
...
void Stepper::StepperHandler() {
  ...
  if (!current_block) {
    ...
    if ((current_block = planner.get_current_block())) {
      trapezoid_generator_reset();//中断时间计算在此函数中
  ...
  for (uint8_t i = step_loops; i--;) {
    ...
    #define PULSE_START(AXIS) \
      ...
      if (_COUNTER(AXIS) > 0) { _APPLY_STEP(AXIS)(!_INVERT_STEP_PIN(AXIS),0); for(volatile int i_axis=0;i_axis<gCfgItems.pulseDelay;i_axis++);}
    ...
    #define PULSE_STOP(AXIS) \
      if (_COUNTER(AXIS) > 0) { \
        ...
        _APPLY_STEP(AXIS)(_INVERT_STEP_PIN(AXIS),0); for(volatile int i_axis=0;i_axis<gCfgItems.pulseDelay;i_axis++);\
      }

    // Advance the Bresenham counter; start a pulse if the axis needs a step
    #define Z_PULSE_START(AXIS) \
      _COUNTER(AXIS) += current_block->steps[_AXIS(AXIS)]; \
      if (_COUNTER(AXIS) > 0) { Z_APPLY_STEP(!_INVERT_STEP_PIN(AXIS),0);for(volatile int iz_axis=0;iz_axis<gCfgItems.pulseDelay;iz_axis++);}

    // Stop an active pulse, reset the Bresenham counter, update the position
    #define Z_PULSE_STOP(AXIS) \
      if (_COUNTER(AXIS) > 0) { \
        _COUNTER(AXIS) -= current_block->step_event_count; \
        count_position[_AXIS(AXIS)] += count_direction[_AXIS(AXIS)]; \
        Z_APPLY_STEP(_INVERT_STEP_PIN(AXIS),0); for(volatile int i_axis=0;i_axis<gCfgItems.pulseDelay;i_axis++);\
      }
