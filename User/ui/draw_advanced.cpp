...
static BUTTON_STRUCT PowerOffDet_PWC,PowerOffDet_MKS_UPS,PausePos,PausePosArrow;
...
static void btnHandle_rest()
{
    ...
    PowerOffDet_MKS_UPS.btnHandle = 0;
	PausePos.btnHandle = 0;
    	...
    	case WM_NOTIFY_PARENT:
    		if(pMsg->Data.v == WM_NOTIFICATION_RELEASED)
    		{
    			...
    			else if(pMsg->hWinSrc == PowerOffDet_DET.btnHandle)
    			{
					...
					BUTTON_SetBmpFileName(PowerOffDet_MKS_UPS.btnHandle, "bmp_mks_ups.bin",1);
    			}
			...
			else if(pMsg->hWinSrc == PowerOffDet_MKS_UPS.btnHandle)
    			{
    				gCfgItems.insert_det_module=2;
					epr_write_data(EPR_INSERT_DET_MODULE_TYPE, (uint8_t *)&gCfgItems.insert_det_module,1);
					BUTTON_SetBmpFileName(PowerOffDet_DET.btnHandle, "bmp_det.bin",1);		  
					BUTTON_SetBmpFileName(PowerOffDet_PWC.btnHandle, "bmp_pwc.bin",1);
					BUTTON_SetBmpFileName(PowerOffDet_MKS_UPS.btnHandle, "bmp_mks_ups_sel.bin",1);
    			}
			...
			else if((pMsg->hWinSrc == PausePos.btnHandle)||
...
{   
    ...
    if(current_page != 1)
    {
        PowerOffDet_text.btnHandle = BUTTON_CreateEx(10,50,230,40,hAdvancedWnd, BUTTON_CF_SHOW, 0, alloc_win_id());
        PowerOffDet_DET.btnHandle = BUTTON_CreateEx(240,50+5,VALUE_DEFAULT_X,VALUE_DEFAULT_Y,hAdvancedWnd, BUTTON_CF_SHOW, 0, alloc_win_id());
        PowerOffDet_PWC.btnHandle = BUTTON_CreateEx(320,50+5,VALUE_DEFAULT_X,VALUE_DEFAULT_Y,hAdvancedWnd, BUTTON_CF_SHOW, 0, alloc_win_id());
	 PowerOffDet_MKS_UPS.btnHandle = BUTTON_CreateEx(400,50+5,VALUE_DEFAULT_X,VALUE_DEFAULT_Y,hAdvancedWnd, BUTTON_CF_SHOW, 0, alloc_win_id());
		...
		else if(gCfgItems.insert_det_module==0)
		{
        	BUTTON_SetBmpFileName(PowerOffDet_DET.btnHandle, "bmp_det.bin",1);        
        	BUTTON_SetBmpFileName(PowerOffDet_PWC.btnHandle, "bmp_pwc_sel.bin",1);
		BUTTON_SetBmpFileName(PowerOffDet_MKS_UPS.btnHandle, "bmp_mks_ups.bin",1);
	 ...
	 BUTTON_SetBitmapEx(PowerOffDet_MKS_UPS.btnHandle,0,&bmp_struct70X28,0,0);
    ...
    else
    {
	 ...
	 PausePos.btnHandle = BUTTON_CreateEx(10,100,450,40,hAdvancedWnd, BUTTON_CF_SHOW, 0, alloc_win_id());
	 PausePosArrow.btnHandle = BUTTON_CreateEx(460,100,10,40,hAdvancedWnd, BUTTON_CF_SHOW, 0, alloc_win_id());
	 ...
	 BUTTON_SetBmpFileName(PausePos.btnHandle,NULL,1);
        
        BUTTON_SetBitmapEx(MotorEnablePinArrow.btnHandle,0,&bmp_struct10x17,0,12);
	 BUTTON_SetBitmapEx(PausePosArrow.btnHandle,0,&bmp_struct10x17,0,12);

        BUTTON_SetTextAlign(MotorEnablePin_text.btnHandle,GUI_TA_LEFT|GUI_TA_VCENTER );
	 BUTTON_SetTextAlign(PausePos.btnHandle,GUI_TA_LEFT|GUI_TA_VCENTER );
     ...
     if(gCfgItems.multiple_language != 0)
     {
        ...
        else
        {
	     ...
	     BUTTON_SetText(PausePos.btnHandle,machine_menu.PausePosText);
        }
