...
volatile uint8_t gcode_disp_update_flag=1;
...
*/
	...
	{
		gCfgItems.touch_adj_flag = 0;
		gCfgItems.touch_adj_xMin = 191;
		gCfgItems.touch_adj_xMax = 3924;
		gCfgItems.touch_adj_yMin = 3863;
		gCfgItems.touch_adj_yMax = 225;	
	}
	else
	{
		gCfgItems.touch_adj_flag = 0;
		gCfgItems.touch_adj_xMin = 3924;
		gCfgItems.touch_adj_xMax = 191;
		gCfgItems.touch_adj_yMin = 225;
		gCfgItems.touch_adj_yMax = 3863;			
	}
    }
}
...
void GUI_RefreshPage()
{
		...
		case BABY_STEP_UI:
			...
			break;
		case GCODE_COMMAND_UI:
			if(gcode_disp_update_flag)
			{
				gcode_disp_update_flag = 0;
				memset(buf,0,sizeof(buf));
				if(popFIFO(&gcodeCmdRxFIFO,  (unsigned char *)buf) == fifo_ok)
				{
					uart_tx.data_disp_row++;
					if(uart_tx.data_disp_row > 10)
					{
						GUI_SetColor(0x000000);
						GUI_FillRect(0, titleHeight+8+23, LCD_WIDTH, LCD_HEIGHT);
						uart_tx.data_disp_row = 1;
						GUI_SetColor(gCfgItems.title_color);
					}
					if(strlen(buf)>=50)
					{
						...
						strcat(buf2,"\n  ");
						strcat(buf2,&buf[50]);
						GUI_DispStringAt((char *)buf2, 10, titleHeight+8+uart_tx.data_disp_row*22);
						uart_tx.data_disp_row++;
					}
					else
					{
						GUI_DispStringAt((char *)buf, 10, titleHeight+8+uart_tx.data_disp_row*22);
					}
				}
			}
	    		break;
