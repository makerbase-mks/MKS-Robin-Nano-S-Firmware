				...
				else if(pMsg->hWinSrc == buttonInc.btnHandle)
				{
					if(gCfgItems.curTempType == 0)
					{
						...
						if(gCfgItems.curSprayerChoose == 0)
						{
							...
							if((int)thermalManager.target_temperature[gCfgItems.curSprayerChoose] > (mksCfg.heater_0_maxtemp- (15)))
							{
								thermalManager.target_temperature[gCfgItems.curSprayerChoose] = (float)mksCfg.heater_0_maxtemp - (15);								
							}
						}
						else
						{
							if((int)thermalManager.target_temperature[gCfgItems.curSprayerChoose] > (mksCfg.heater_1_maxtemp- (15)))
							{
								thermalManager.target_temperature[gCfgItems.curSprayerChoose] = (float)mksCfg.heater_1_maxtemp - (15);
					...
					else
					{
						...
						if((int)thermalManager.target_temperature_bed > mksCfg.bed_maxtemp- (15))
						{
							thermalManager.target_temperature_bed = (float)mksCfg.bed_maxtemp - (15);
						}
