...
void LCD_CtrlLinesConfig(void)
{
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_13;//PB15 推挽输出,控制背光
...
void LCD_FSMCConfig(void)
{
  ...
  readWriteTiming.FSMC_DataSetupTime = 8;			//数据保存时间为60个HCLK	=6*60=360ns
  ...
  FSMC_NORSRAMInitStructure.FSMC_WriteTimingStruct = &readWriteTiming;  //写时序  &readWriteTiming;//  
#else
	readWriteTiming.FSMC_AddressSetupTime = 15;	   //地址建立时间（ADDSET）为16个HCLK 1/168M=6ns*16=96ns  
	readWriteTiming.FSMC_AddressHoldTime = 0x00;   //地址保持时间（ADDHLD）模式A未用到	  
  ...
  FSMC_NORSRAMInitStructure.FSMC_WriteBurst = FSMC_WriteBurst_Disable; 
  FSMC_NORSRAMInitStructure.FSMC_ReadWriteTimingStruct = &readWriteTiming; //读写时序
  FSMC_NORSRAMInitStructure.FSMC_WriteTimingStruct = &writeTiming;  //写时序  &readWriteTiming;//
#endif
	...
	//LCD_Clear(WHITE);
	   LCD_WrtReg(0x0029); //开启显示
		...
		else if(DeviceCode==0x9488)
		{
			...
			ILI9488_WriteCmd(0xF4);      //解决电荷残留问题
			ILI9488_WriteData(0x00);
			ILI9488_WriteData(0x00);
			ILI9488_WriteData(0x0f);	
