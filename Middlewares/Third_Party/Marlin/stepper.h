...
#define ENABLE_STEPPER_DRIVER_INTERRUPT()   HAL_TIM_Base_Start_IT(&htim2)    //?a?§?è?D?§?§?§??a?a???ì?§o?¨¤????D??
#define DISABLE_STEPPER_DRIVER_INTERRUPT() 	HAL_TIM_Base_Stop_IT(&htim2)		//?a?§?è?D?§?§?§??a1????ì?§o?¨¤????D??
  ...
  private:
    ...
    static volatile uint32_t step_events_completed; // The number of step events executed in the current block
      ...
      //#define _NEXT_ISR(T) OCR1A = T
			#define _NEXT_ISR(T)	do{OCR1A = T;  if(TCNT1>OCR1A) TCNT1 = OCR1A-1;}while(0)
    ...
    FORCE_INLINE static unsigned short calc_timer_interval(unsigned short step_rate) {
      ...
      if (timer < 100) { // (20kHz - this should never happen)
        ...
        MYSERIAL.println(step_rate);
  		...
  		timer = (uint16_t)(F_CPU() / (step_rate * (2*TICK_TIMER_PRESCALER+2)));		/*--mks--*/ 
