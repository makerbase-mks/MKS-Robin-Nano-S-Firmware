...
volatile uint8_t read_lcd_reg_flag = 0;
#endif
...
#if 1//ENABLED(AUTO_BED_LEVELING_UBL)
  #include "ubl.h"
...
uint8_t commands_in_queue = 0,commands_in_queue_bak = 0;
static uint8_t cmd_queue_index_r = 0, // Ring buffer read position
               cmd_queue_index_w = 0; // Ring buffer write position
uint8_t cmd_queue_index_r_bak = 0; // Ring buffer read position
               //cmd_queue_index_w_bak = 0; // Ring buffer write position          
#if ENABLED(M100_FREE_MEMORY_WATCHER)
...
void soft_endstop_min_init()
{
    ...
    {
        soft_endstop_min[0]= X_MIN_BED_IS_KINEMATIC;
        soft_endstop_min[1]= Y_MIN_BED_IS_KINEMATIC;
        soft_endstop_min[2]= Z_MIN_POS;

        soft_endstop_max[0] = X_MAX_BED_IS_KINEMATIC;
        soft_endstop_max[1] = Y_MAX_BED_IS_KINEMATIC;
        soft_endstop_max[2] = Z_MAX_POS;
    }
    else
    {
        soft_endstop_min[0]= X_MIN_BED_IS_CARTESIAN;
        soft_endstop_min[1]= Y_MIN_BED_IS_CARTESIAN;
        soft_endstop_min[2]= Z_MIN_POS;

        soft_endstop_max[0] = X_MAX_BED_IS_CARTESIAN;
        soft_endstop_max[1] = Y_MAX_BED_IS_CARTESIAN;
        soft_endstop_max[2] = Z_MAX_POS;
    }        
}
...
#if 1//ENABLED(AUTO_BED_LEVELING_BILINEAR)
/*
  #if ENABLED(DELTA)
...
#if ENABLED(SDSUPPORT)
  uint8_t sd_read_error_flg=0;
  uint8_t sd_read_cnt=0;
  
  void display_sd_error()
  {
    char buf_sd_err[30]={0};
    ...
    if(sd_read_error_flg == 1)
    {
        ...
        GUI_DispStringAt(buf_sd_err,400, TITLE_YPOS);
        GUI_SetBkColor(gCfgItems.background_color);
...
bool get_target_extruder_from_command(const uint16_t code) {
  ...
  void update_software_endstops(const AxisEnum axis) {
	...
	#else
    if (MACHINETPYE == DELTA)//ENABLED(DELTA)
    {
      soft_endstop_min[axis] = base_min_pos(axis);
...
void do_blocking_move_to(const float &rx, const float &ry, const float &rz, const float &fr_mm_s/*=0.0*/) {
  ...
  if(MACHINETPYE == DELTA)
  {
    if (!position_is_reachable_IS_KINEMATIC(rx, ry)) return;
...
static void clean_up_after_endstop_or_probe_move() {
  ...
  // TRIGGERED_WHEN_STOWED_TEST can easily be extended to servo probes, ... if needed.
    ...
    #if 1 //ENABLED(Z_MIN_PROBE_ENDSTOP)
  ...
  static bool do_probe_move(const float z, const float fr_mm_m) {
    ...
    */
      ...
      {
        probe_triggered = TEST(Endstops::endstop_hit_bits,Z_MIN);
      }
      else
      {
        probe_triggered = TEST(Endstops::endstop_hit_bits,Z_MIN_PROBE);
      }
  ...
  float probe_pt(const float &rx, const float &ry, const bool stow, const uint8_t verbose_level, const bool probe_relative=true) {
    ...
    if (probe_relative) {
      if(MACHINETPYE & IS_KINEMATIC)
      {
        if (!position_is_reachable_by_probe_IS_KINEMATIC(rx, ry)) return NAN;  // The given position is in terms of the probe
      }
      else
      {
        if (!position_is_reachable_by_probe_IS_CARTESIAN(rx, ry)) return NAN;  // The given position is in terms of the probe
      }
    ...
    else
    {
        if(MACHINETPYE & IS_KINEMATIC)
        {
            if (!position_is_reachable_IS_KINEMATIC(nx, ny)) return NAN;        // The given position is in terms of the nozzle
        }
        else
        {
            if (!position_is_reachable_IS_CARTESIAN(nx, ny)) return NAN;        // The given position is in terms of the nozzle            
        }
     }
    //const 
        float nz;/* =
    ...
    */
    if(MACHINETPYE ==  DELTA)
    {
        nz = min(current_position[Z_AXIS], delta_clip_start_height);
    }
    else
    {
...
*/
    if(BED_LEVELING_METHOD == AUTO_BED_LEVELING_BILINEAR)
        can_change = (!enable || leveling_is_valid());
    else
        can_change = true;
...
#if 1//ENABLED(AUTO_BED_LEVELING_BILINEAR) || ENABLED(MESH_BED_LEVELING)
...
static void do_homing_move(const AxisEnum axis, const float distance, const float fr_mm_s=0.0) {
  ...
  if((Z_HOME_DIR < 0) && (Z_MIN_PROBE_PIN_MODE == Z_MIN_PROBE_USES_Z_MIN_ENDSTOP_PIN)){	/*--mks cfg--*/
    deploy_bltouch = (axis == Z_AXIS && distance < 0);
...
#define CAN_HOME(A) \
  ...
  */
   ...
   */
  #if 1//ENABLED(X_DUAL_ENDSTOPS) || ENABLED(Y_DUAL_ENDSTOPS) || ENABLED(Z_DUAL_ENDSTOPS)
...
void gcode_get_destination() {
  LOOP_XYZE(i) {
    if (parser.seen(axis_codes[i])) {
      ...
      destination[i] = v;
      /*
      if((axis_codes[i]=='Z')&&(gCfgItems.breakpoint_reprint_flg==1))
        gCfgItems.breakpoint_resume_z_pos=v;
        */
    }
    else
...
/*
    ...
    {
        if (destination[Z_AXIS] < gCfgItems.breakpoint_z_pos) 
        {
            gCfgItems.had_breakpoint=1;
            gCfgItems.breakpoint_e_position=LROUND(destination[E_AXIS] * planner.axis_steps_per_mm[E_AXIS]);
...
if(MACHINETPYE & IS_KINEMATIC)
{
    if (position_is_reachable_IS_KINEMATIC(destination[X_AXIS], destination[Y_AXIS])) {
...
{
    if (position_is_reachable_IS_CARTESIAN(destination[X_AXIS], destination[Y_AXIS])) {
...
inline void gcode_G28(const bool always_home_all) {
  ...
  //#if HAS_LEVELING
  	//const 
  	bool ubl_state_at_entry;
    //#if ENABLED(AUTO_BED_LEVELING_UBL)
    if(BED_LEVELING_METHOD == AUTO_BED_LEVELING_UBL)
    {
    	ubl_state_at_entry = planner.leveling_active;
    }
    //#endif
...
void home_all_axes() { gcode_G28(true); }
...
#if 1//ENABLED(MESH_BED_LEVELING) || ENABLED(PROBE_MANUALLY)
  ...
  void mbl_mesh_report() {
    ...
    SERIAL_PROTOCOL((int)GRID_MAX_POINTS_X);
    SERIAL_PROTOCOLCHAR(',');
    SERIAL_PROTOCOL((int)GRID_MAX_POINTS_Y);
    SERIAL_EOL();
...
//#elif OLDSCHOOL_ABL
  ...
  inline void gcode_G29() {
    ...
    //#endif
      ...
      //#if ENABLED(AUTO_BED_LEVELING_LINEAR)
        ...
        ABL_VAR bool do_topography_map=0;
      //#else // Bilinear
      ...
      //#endif
      if(! (BED_LEVELING_METHOD&ABL_PLANAR)) abl2 = GRID_MAX_POINTS;
      ...
      //#elif ENABLED(AUTO_BED_LEVELING_LINEAR)
        ...
        ABL_VAR float *eqnAMatrix=NULL,//[GRID_MAX_POINTS * 3], // "A" matrix of the linear system of equations
                      *eqnBVector=NULL,//[GRID_MAX_POINTS],     // "B" vector of Z points
                      mean;
		eqnAMatrix = (float *)malloc((GRID_MAX_POINTS * 3 + 1) * sizeof(float));
		if(eqnAMatrix == NULL)
			{
			Running = false;
			kill("malloc eqnAMatrix err!");
			}
		for(int i=0;i<GRID_MAX_POINTS * 3 + 1;i++)
            *(eqnAMatrix+i) = 0.0;

		
		eqnBVector = (float *)malloc((GRID_MAX_POINTS + 1) * sizeof(float));
		if(eqnBVector == NULL)
		{
			Running = false;
			kill("malloc eqnBVector err!");
		}
		for(int i=0;i<GRID_MAX_POINTS + 1;i++) 	*(eqnBVector+i) = 0.0;        
      //#endif
    ...
    if (!g29_in_progress) {
      ...
      //
        ...
        while (abl_probe_index < abl2) {
          ...
          if(MACHINETPYE & IS_KINEMATIC)
            if (position_is_reachable_IS_KINEMATIC(xProbe, yProbe)) break;
          else
            if (position_is_reachable_IS_CARTESIAN(xProbe, yProbe)) break;
        ...
        for (uint8_t PR_OUTER_VAR = 0; PR_OUTER_VAR < PR_OUTER_END && !isnan(measured_z); PR_OUTER_VAR++) {
          ...
          for (int8_t PR_INNER_VAR = inStart; PR_INNER_VAR != inStop; PR_INNER_VAR += inInc) {
            ...
            if(MACHINETPYE & IS_KINEMATIC)
              ...
              if (!position_is_reachable_by_probe_IS_KINEMATIC(xProbe, yProbe)) continue;
            //#endif
            
            int16_t randData = rand()%200;
            if(randData > 100)  randData = 100 - randData;

            measured_z = faux ? 0.001 * randData/*random(-100, 101) */: probe_pt(xProbe, yProbe, stow_probe_after_each, verbose_level);
  ...
  inline void gcode_G30() {
    ...
    if(MACHINETPYE & IS_KINEMATIC)
        if (!position_is_reachable_by_probe_IS_KINEMATIC(xpos, ypos)) return;
    else
        if (!position_is_reachable_by_probe_IS_CARTESIAN(xpos, ypos)) return;
  ...
  inline void gcode_G33() {
    ...
    if (!_1p_calibration && !_0p_calibration) {  // test if the outer radius is reachable
      LOOP_CAL_RAD(axis) {
        ...
        if(MACHINETPYE & IS_KINEMATIC)
        {
        if (!position_is_reachable_IS_KINEMATIC(cos(a) * r, sin(a) * r)) {
          SERIAL_PROTOCOLLNPGM("?(M665 B)ed radius is implausible.");
          return;
        }
        else
        {
        if (!position_is_reachable_IS_CARTESIAN(cos(a) * r, sin(a) * r)) {
          SERIAL_PROTOCOLLNPGM("?(M665 B)ed radius is implausible.");
          return;        
        }
        }
  ...
  inline void gcode_G42() {
    ...
    if (IsRunning()) {
      ...
      {
        if(BED_LEVELING_METHOD & AUTO_BED_LEVELING_BILINEAR)
            destination[X_AXIS] = _GET_MESH_X_ABL(ix);
        if(BED_LEVELING_METHOD & AUTO_BED_LEVELING_UBL)
            destination[X_AXIS] = _GET_MESH_X_UBL(ix);
        if(BED_LEVELING_METHOD & MESH_BED_LEVELING)
            destination[X_AXIS] = _GET_MESH_X_MBL(ix);        
      }
      if (hasJ) 
      {
        if(BED_LEVELING_METHOD & AUTO_BED_LEVELING_BILINEAR)
            destination[Y_AXIS] = _GET_MESH_Y_ABL (iy);
        if(BED_LEVELING_METHOD & AUTO_BED_LEVELING_UBL)
            destination[Y_AXIS] = _GET_MESH_Y_UBL(iy);
        if(BED_LEVELING_METHOD & MESH_BED_LEVELING)
            destination[Y_AXIS] = _GET_MESH_Y_MBL(iy);        
      }         
      if (parser.boolval('P')) {
...
*/
  ...
  inline void gcode_M48() {
    ...
    //#else
     ...
     if(MACHINETPYE & IS_KINEMATIC)
        {
      if (!position_is_reachable_by_probe_IS_KINEMATIC(X_probe_location, Y_probe_location)) {
        ...
        {
      if (!position_is_reachable_by_probe_IS_CARTESIAN(X_probe_location, Y_probe_location)) {
        SERIAL_PROTOCOLLNPGM("? (X,Y) location outside of probeable radius.");
        return;
      }
        }        
...
*/
	...
	if(BED_LEVELING_METHOD == AUTO_BED_LEVELING_UBL)
		...
		was_enabled = planner.leveling_active;
        ...
        if (n_legs) {
          ...
          for (uint8_t l = 0; l < n_legs - 1; l++) {
            ...
            else
            {
              ...
              if(MACHINETPYE & IS_KINEMATIC)
                {
              while (!position_is_reachable_by_probe_IS_KINEMATIC(X_current, Y_current)) {
                X_current *= 0.8;
                Y_current *= 0.8;
                if (verbose_level > 3) {
                 ...
                 {
              while (!position_is_reachable_by_probe_IS_CARTESIAN(X_current, Y_current)) {
                X_current *= 0.8;
                Y_current *= 0.8;
                if (verbose_level > 3) {
                  SERIAL_ECHOPAIR("Pulling point towards center:", X_current);
...
#if 1//ENABLED(G26_MESH_VALIDATION)
...
inline void gcode_M109() {
  ...
  #if WATCH_HOTENDS
    thermalManager.start_watching_heater(target_extruder);
  #endif
...
inline void gcode_M115() {
  SERIAL_PROTOCOLLNPGM("Robin_nanoF4_V2.2.0.0");
...
//#if HAS_SERVOS
  ...
  inline void gcode_M280() {
    ...
    if (WITHIN(servo_index, 0, 0/*NUM_SERVOS - 1*/)) {
    if (parser.seen('S'))
...
void quickstop_stepper() {
   ...
   *   M421 I<xindex> J<yindex> Q<offset>
   */
  inline void gcode_M421_AUTO_BED_LEVELING_BILINEAR() {
   ...
   *   M421 C Q<offset>
   */
  inline void gcode_M421_AUTO_BED_LEVELING_UBL() {
    int8_t ix = parser.intval('I', -1), iy = parser.intval('J', -1);
    const bool hasI = ix >= 0,
               hasJ = iy >= 0,
               hasC = parser.seen('C'),
      ...
      if (tmp_extruder != active_extruder) {
        ...
        set_destination_from_current();
		  ...
		  if(BED_LEVELING_METHOD&ABL_PLANAR)
			...
			xydiff[0] = offset_vec.x;
			xydiff[1] = offset_vec.y;
          ...
          //#else // !ABL_PLANAR
            ...
            if(gCfgItems.breakpoint_flg!=1)
            {
			    xydiff[0] = hotend_offset[X_AXIS][tmp_extruder] - hotend_offset[X_AXIS][active_extruder];
			    xydiff[1] = hotend_offset[Y_AXIS][tmp_extruder] - hotend_offset[Y_AXIS][active_extruder];
            }
  ...
  switch (parser.command_letter) {
    case 'G': switch (parser.codenum) {
      ...
      case 1:
        //#if IS_SCARA
        if(MACHINETPYE&IS_SCARA)
          gcode_G0_G1_SCARA(parser.codenum == 0);
        //#else
        else
          gcode_G0_G1();
        //#endif
      ...
      case 28: // G28: Home all axes, one at a time
		...
		if((BED_LEVELING_METHOD & HAS_LEVELING) && (BED_LEVELING_METHOD != AUTO_BED_LEVELING_UBL))
		{
			 set_bed_leveling_enabled(true);
				...
				if((MACHINETPYE != DELTA) && (BED_LEVELING_METHOD&HAS_FADE))	//mks_delta
				 { 
				 	if (parser.seenval('Z')) set_z_fade_height(parser.value_linear_units()/*code_value_linear_units()*/);
				 }
			#endif
          }
        break;
      ...
      //#if HAS_MESH
        case 421: // M421: Set a Mesh Bed Leveling Z coordinate
          if(BED_LEVELING_METHOD & MESH_BED_LEVELING)
          	...
          	gcode_M421_AUTO_BED_LEVELING_BILINEAR();
          else if(BED_LEVELING_METHOD & AUTO_BED_LEVELING_UBL)
            gcode_M421_AUTO_BED_LEVELING_UBL();
          break;
      //#endif
...
#if 1//IS_CARTESIAN
#if 1//ENABLED(SEGMENT_LEVELED_MOVES)
...
//#elif ENABLED(AUTO_BED_LEVELING_BILINEAR)
  ...
  inline bool prepare_kinematic_move_to(const float (&rtarget)[XYZE]) {
    ...
    if(MACHINETPYE & IS_KINEMATIC)
    {
        if (!position_is_reachable_IS_KINEMATIC(rtarget[X_AXIS], rtarget[Y_AXIS])) return true;
    }
    else
    {
        if (!position_is_reachable_IS_CARTESIAN(rtarget[X_AXIS], rtarget[Y_AXIS])) return true;
    }
    // Remaining cartesian distances
        ...
        oldA = delta[A_AXIS]; oldB = delta[B_AXIS];
          ...
          if (gCfgItems.breakpoint_reprint_flg == 1) 
          {
              if (rtarget[Z_AXIS] < gCfgItems.breakpoint_z_pos) 
              {
                  ...
                  gCfgItems.had_breakpoint=1;
                  gCfgItems.breakpoint_e_position=LROUND(raw[E_AXIS] * planner.axis_steps_per_mm[E_AXIS]);
...
void prepare_move_to_destination() {
  ...
  if((BED_LEVELING_METHOD &AUTO_BED_LEVELING_UBL) && (MACHINETPYE == DELTA /*|| ENABLED(SEGMENT_LEVELED_MOVES)*/))
  {
    if(ubl.prepare_segmented_line_to(destination, MMS_SCALED(feedrate_mm_s)))return;
  } 
  else if(MACHINETPYE & IS_KINEMATIC)
  {
    if(prepare_kinematic_move_to(destination))return;
  }  
  else//if(MACHINETPYE & IS_CARTESIAN)
  {
   ...
   * Integrated into Marlin and slightly restructured by Joachim Cerny.
   */
  void inverse_kinematics_MORGAN_SCARA(const float raw[XYZ]) {
...
void idle(
  ...
  #endif
  if(read_lcd_reg_flag == 1) {
  	read_lcd_reg_flag = 0;
  	get_lcd_reg_value();
	if(compare_lcd_reg_value() < 0) {
		//no_reflesh_ram_value = 1;
		//if(disp_state == PRINTING_UI) printing_no_reflesh_ram_value = 1;
		lcd_reInit();
		//no_reflesh_ram_value = 0;
		//GUI_Exec();
	}
  }
}
...
extern volatile uint8_t has_adjust_speed;
extern volatile uint16_t resume_printed_time;

extern uint8_t print_finish_start_timer;
extern uint32_t print_finish_timer_count;
extern uint8_t print_finish_close_machine;

extern volatile uint8_t gcode_disp_update_flag;
