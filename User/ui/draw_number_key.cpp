...
void set_value_confirm()
{
	...
	case HotBed_min:
         ...
         case HotBed_pid_d:
            ...
            break;  
	  case pause_pos_x:
	  	mksReprint.mks_pausePrint_x = mksCfg.filament_change_x_pos=atof(key_value);
		epr_write_data((int)EPR_FILAMENT_CHANGE_X_POS, (uint8_t*)&mksCfg.filament_change_x_pos, sizeof(mksCfg.filament_change_x_pos));
	  	break;
	  case pause_pos_y:
	  	mksReprint.mks_pausePrint_y = mksCfg.filament_change_y_pos=atof(key_value);
		epr_write_data((int)EPR_FILAMENT_CHANGE_Y_POS, (uint8_t*)&mksCfg.filament_change_y_pos, sizeof(mksCfg.filament_change_y_pos));
	  	break;
	 case pause_pos_z:
	  	mksReprint.mks_pausePrint_z = mksCfg.filament_change_z_add=atof(key_value);
		epr_write_data((int)EPR_FILAMENT_CHANGE_Z_ADD, (uint8_t*)&mksCfg.filament_change_z_add, sizeof(mksCfg.filament_change_z_add));
	  	break;
    }    
}
void disp_value()
{
	...
	case HotBed_min:
	 ...
	 case pause_pos_x:
	 memset(cmd_code,0,sizeof(cmd_code));
        sprintf(cmd_code,"%.3f",mksReprint.mks_pausePrint_x);
	 break;
	 case pause_pos_y:
	 memset(cmd_code,0,sizeof(cmd_code));
        sprintf(cmd_code,"%.3f",mksReprint.mks_pausePrint_y);
	 break;
	 case pause_pos_z:
	 memset(cmd_code,0,sizeof(cmd_code));
        sprintf(cmd_code,"%.3f",mksReprint.mks_pausePrint_z);
	 break;
    }
    memset(key_value,0,sizeof(key_value));
